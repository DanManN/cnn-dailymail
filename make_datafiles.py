...
# These are the number of .story files we expect there
# to be in cnn_stories_dir and dm_stories_dir
...
def tokenize_stories(stories_dir, tokenized_stories_dir):
    """Maps a whole directory of .story files to a
    tokenized version using Stanford CoreNLP Tokenizer"""
...
def get_art_abs(story_file):
    ...
    # captions don't end in periods; consequently they end up
    # in the body of the article as run-on sentences)
    ...
    abstract = ' '.join(["%s %s %s"
                         % (SENTENCE_START, sent, SENTENCE_END)
                         for sent in highlights])

    return bytes(article, 'utf-8'), bytes(abstract, 'utf-8')


def write_to_bin(url_file, out_file, makevocab=False):
    """Reads the tokenized .story files corresponding to
    the urls listed in the url_file and writes them to a out_file."""
    ...
    with open(out_file, 'wb') as writer:
        for idx, s in enumerate(story_fnames):
            ...
            # Look in the tokenized story dirs to find the
            # .story file corresponding to this url
            if os.path.isfile(os.path.join(cnn_tokenized_stories_dir, s)):
                story_file = os.path.join(cnn_tokenized_stories_dir, s)
            elif os.path.isfile(os.path.join(dm_tokenized_stories_dir, s)):
                story_file = os.path.join(dm_tokenized_stories_dir, s)
            else:
                print("""Error: Couldn't find tokenized story file %s in
                      either tokenized story directories %s and %s.
                      Was there an error during tokenization?"""
                      % (s,
                         cnn_tokenized_stories_dir,
                         dm_tokenized_stories_dir))
                # Check again if tokenized stories directories
                # contain correct number of files
                print("""Checking that the tokenized stories directories
                      %s and %s contain correct number of files..."""
                ...
                raise Exception("""Tokenized stories directories %s and %s
                                contain correct number of files but story
                                file %s found in neither."""
                                % (cnn_tokenized_stories_dir,
                                   dm_tokenized_stories_dir,
                                   s))
            ...
            tf_example.features.feature['article'] \
                .bytes_list.value.extend([article])
            tf_example.features.feature['abstract'] \
...
def check_num_stories(stories_dir, num_expected):
    ...
    if num_stories != num_expected:
        raise Exception("""stories directory %s contains %i files
                        but should contain %i"""
...
if __name__ == '__main__':
    ...
    # Read the tokenized stories, do a little
    ...
    # smaller chunks, each containing e.g. 1000 examples, and
    # saves them in finished_files/chunks
    chunk_all()
